...
if ($action == "main")
{
    ...
    elseif ($userdata['isbuilding'] == "1" and $userdata['endbuildtime'] <= time())
    {
        ...
        {
            $db->query("UPDATE cc" . $n .
                "_countries SET  bid='0', isbuilding='0', startbuildtime='0', endbuildtime='0' WHERE islandid='" . $userdata['islandid'] .
                "'");
    ...
    $new_found = array();
    $result_buildings = $db->query("SELECT * FROM cc" . $n . "_buildings where race = " . $userdata['rassenid'] .
        " and (tabless !='' and tabless !='0' ) and( require1 <= '" . $userdata['build_town'] . "' and require2 <= '" . $userdata['build_explore'] .
        "')ORDER BY p ASC");
    while ($row_buildings = $db->fetch_array($result_buildings))
    {
        ...
        if (isset($show_bau) and $show_bau == 1)
        {
            if ($in_bau == $build_bid)
            {
                ...
                $message = make_timebanner($userdata['startbuildtime'], $userdata['endbuildtime'], $build_bid, "buildings.php") .
                    "<br><a href=\"$cancelURL\">" . $buildingabort . "</a>";
            }
            else
            {
                $es_wird_gebaut = 1;
        ...
        else
        {
            ...
            else
            {
                ...
                if ($tofew_res1 > 0) $message = $message . "<br>" . $op_set_n_res1 . ": " . $tofew_res1;
                $tofew_res2 = $res2 - $userdata['res2'];
                if ($tofew_res2 > 0) $message = $message . "<br>" . $op_set_n_res2 . ": " . $tofew_res2;
                $tofew_res3 = $res3 - $userdata['res3'];
                if ($tofew_res3 > 0) $message = $message . "<br>" . $op_set_n_res3 . ": " . $tofew_res3;
                $tofew_res4 = $res4 - $userdata['res4'];
                if ($tofew_res4 > 0) $message = $message . "<br>" . $op_set_n_res4 . ": " . $tofew_res4;
            }
...
if ($action == "build")
{
    ...
    if ($userdata['res1'] >= $res1 && $userdata['res2'] >= $res2 && $userdata['res3'] >= $res3 && $userdata['res4'] >= $res4 &&
        $userdata['bid'] == 0)
    {
        $db->query("UPDATE cc" . $n . "_countries SET bid='$bid', res1=res1-'$res1', res2=res2-'$res2', res3=res3-'$res3', res4=res4-'$res4', startbuildtime='$startbuildtime', endbuildtime='$endbuildtime', isbuilding='1', usesize=usesize+" .
            $row['size'] . " WHERE islandid='" . $userdata['activeid'] . "'");
...
if ($action == "remove")
{
    ...
    $db->query("UPDATE cc" . $n . "_countries SET usesize=usesize-'" . $row['size'] . "', $row[tabless]=$row[tabless]-'1', res1=res1+'$res1', res2=res2+'$res2', res3=res3+'$res3', res4=res4+'$res4' WHERE islandid='" .
        $userdata['activeid'] . "'");
...
if ($action == "del")
{
    ...
    $db->query("UPDATE cc" . $n . "_countries SET bid='0', usesize=usesize-'" . $unew_size .
        "', isbuilding='0', startbuildtime='0', endbuildtime='0', res1=res1+'$res1', res2=res2+'$res2', res3=res3+'$res3', res4=res4+'$res4' WHERE islandid='" .
        $userdata['activeid'] . "'");
    header("LOCATION: buildings.php");
    exit();
}
