...
if ($action == "main")
{
    ...
    $nn = 0;
    $inhalt = array();
    $result = $db->query("SELECT * FROM cc" . $n .
        "_users WHERE blocked='0' AND serveradmin='0' ORDER BY points DESC LIMIT 10");
...
if ($action == "user")
{
    ...
    $result = $db->query("SELECT userid,username,points,allianzid,lastlogin,lastpoints,lastactive,status,umod FROM cc" . $n .
        "_users WHERE blocked='0' ORDER BY points DESC LIMIT $start,30");
    ...
    while ($row = $db->fetch_array($result))
    {
        ...
        $inhalt[$nn]['username'] = generate_messagelink_smal($row['username']) . " " . generate_userlink($row['userid'], $row['username']) .
            $online;
