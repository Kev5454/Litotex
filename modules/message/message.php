...
if ($action == "main")
{
    ...
    /** check folder **/
    if (isset($_REQUEST['folder'])) $folder = intval($_REQUEST['folder']);
    else  $folder = "inbox";
    ...
    $result = $db->query("SELECT * FROM cc" . $n . "_messages WHERE $box AND touserid='" . $userdata['userid'] .
        "' ORDER BY time DESC");
    ...
    while ($row = $db->fetch_array($result))
    {
        ...
        if ($row['pri'] == 1) $pri = "<span class=\"red\">!</span>";
    ...
    $result = $db->query("SELECT * FROM cc" . $n . "_messages WHERE inbox='0' AND archive='1' AND touserid='" . $userdata['userid'] .
        "' ORDER BY time DESC");
...
if ($action == "send")
{
    ...
    $username_r = (isset($_GET['username']) ? c_trim($_GET['username']) : 'unbekannt');
    if ($pmid > 0)
    {
...
if ($action == "submit_send")
{
    ...
    for ($i = 0; $i < count($user_array); $i++)
    {
        ...
        $db->query("INSERT INTO cc" . $n .
            ...
            $userdata['userid'] . "','" . $userid_c . "','" . mysql_real_escape_string($text) . "','" . time() . "','1','1','" .
            mysql_real_escape_string($subject) . "','" . $pri . "')");
...
if ($action == "lookup")
{
    ...
    $db->query("UPDATE cc" . $n . "_messages SET isnew='0' WHERE pmid='$pmid' AND touserid='" . $userdata['userid'] .
        "' AND isnew='1'");
    ...
    elseif ($todo == "2")
    {
        ...
        foreach ($checkbox as $pmid => $box)
        {
            if ($pmid == true)
            {
                $xyz = $xyz + 1;
                settype($pmid, integer);
                $db->unbuffered_query("UPDATE cc" . $n . "_messages SET archive='1', inbox='0' WHERE pmid='" . $pmid .
                    "' AND touserid='" . $userdata['userid'] . "'");
                $update = mysql_query($change);
            }
        }
        header("LOCATION: message.php");
