...
if ($action == "show_forum")
{
    ...
    $new_thema = "<a href=\"forum.php?action=new_threadid&ft_id=$f_id&f_id=$f_id\"><img src=\"" . LITO_IMG_PATH_URL . $modul_name .
        "/new_thema.png\" title=\"neues Thema hinzuf&uuml;gen\" alt=\"neues Thema hinzuf&uuml;gen\" border=\"0\"></a>";
...
if ($action == "create_thread")
{
    ...
    $db->query("INSERT INTO cc" . $n .
        "_forum_topics (si_topic_title,si_topic_create_uid ,si_topic_create_uname ,si_topic_create_time,si_forum_id,si_topic_last_post_time,si_topic_last_post_name,si_topic_last_post_uid) VALUES ('" .
        $ft_titel_new . "','" . $uid . "','" . $uname . "','" . $date . "','" . $ft_id . "','" . $date . "','" . $uname . "','" .
        $uid . "')");
    $ulast_id = $db->insert_id();
    $db->query("INSERT INTO cc" . $n .
        "_forum_posts (si_forum_id ,si_topic_id ,si_poster_id,si_poster_name,si_post_text ,si_post_time,si_poster_ip ) VALUES ('" .
        $f_id . "','" . $ulast_id . "','" . $uid . "','" . $uname . "','." . $ft_titel_text . "','" . $date . "','" . $my_ip .
        "')");

    $db->unbuffered_query("UPDATE cc" . $n .
        "_forum SET si_count_topic  =si_count_topic +1 ,si_count_post=si_count_post+1 WHERE si_forum_id ='" . $ft_id . "' ");
...
if ($action == "show_post")
{
    ...
    while ($row = $db->fetch_array($result))
    {
        ...
        {
            $post_edit_pic = "<a href=\"forum.php?action=edit&ft_id=$ft_id&f_id=$f_id&fp_id=$post_new_id\"><img src=\"" .
                LITO_IMG_PATH_URL . $modul_name . "/edit.png\" title=\"Bearbeiten\" alt=\"Bearbeiten\" border=\"0\"></a>";
        }
        else
        {
            $post_edit_pic = "";
        ...
        if ($post_id_t == $userdata['userid'] || $ali_admin == 1)
        {
            $post_del_pic = "<a href=\"forum.php?action=delete&ft_id=$ft_id&f_id=$f_id&fp_id=$post_new_id\"><img src=\"" .
                LITO_IMG_PATH_URL . $modul_name . "/del.png\" title=\"Löschen\" alt=\"Löschen\" border=\"0\"></a>";
        }
        else
        {
            $post_del_pic = "";
    ...
    $sql = "DELETE From cc" . $n . "_forum_last where forum_id ='" . $f_id . "' AND topic_id ='" . $ft_id .
        "' and user_id = '" . $uid . "'";

    $db->unbuffered_query($sql);
    $db->unbuffered_query("INSERT INTO  cc" . $n . "_forum_last (forum_id,topic_id,user_id,post_id) VALUES ('" . $f_id .
        "','" . $ft_id . "','" . $uid . "','" . $last_post_id . "')");

    $new_thema = "<a href=\"forum.php?action=replay_thread&ft_id=$ft_id&f_id=$f_id\"><img src=\"" . LITO_IMG_PATH_URL . $modul_name .
        "/antworten.png\" title=\"Antwort hinzuf&uuml;gen\" alt=\"Antwort hinzuf&uuml;gen\" border=\"0\"></a>";
    $ft_parent = "<a href=\"forum.php\">Forum</a>" . " / <a href=\"forum.php?action=show_forum&f_id=$f_id\">" .
        get_forum_from_id($f_id) . "</a> / " . get_topic_from_id($ft_id);
...
if ($action == "replay_save")
{
    ...
    $db->query("INSERT INTO cc" . $n .
        "_forum_posts (si_forum_id ,si_topic_id ,si_poster_id,si_poster_name,si_post_text ,si_post_time,si_poster_ip ) VALUES ('" .
        $f_id . "','" . $ft_id . "','" . $uid . "','" . $uname . "','" . $ft_titel_text . "','" . $date . "','" . $my_ip . "')");
    $db->unbuffered_query("UPDATE cc" . $n .
        "_forum_topics SET si_posts_count=si_posts_count  +1, si_topic_last_post_time ='" . $date .
        "', si_topic_last_post_name='" . $uname . "', si_topic_last_post_uid='" . $uid . "' WHERE si_topic_id ='" . $ft_id .
        "' ");
...
if ($action == "edit_save")
{
    ...
    $sql = "DELETE From cc" . $n . "_forum_last where forum_id ='" . $f_id . "' AND topic_id ='" . $ft_id .
        "' and user_id != '" . $uid . "'";
    $db->unbuffered_query($sql);
    $db->unbuffered_query("UPDATE cc" . $n . "_forum_posts SET si_post_text ='" . $ft_titel_text . "' WHERE si_post_id  ='" .
        $fp_id . "' and si_poster_id='" . $uid . "'");
    header("LOCATION: forum.php?action=show_post&f_id=$f_id&ft_id=$ft_id");
    exit();
...
if ($action == "delete")
{
    ...
    if ($ali_admin == 1)
    {
        $sql = "DELETE From cc" . $n . "_forum_posts where  si_forum_id ='" . $f_id . "' AND  si_topic_id ='" . $ft_id .
            "' and  si_post_id='" . $f_p_id . "' ";
        $db->unbuffered_query($sql);
    }
    else
    {
        $sql = "DELETE From cc" . $n . "_forum_posts where  si_forum_id ='" . $f_id . "' AND  si_topic_id ='" . $ft_id .
            "' and  si_post_id='" . $f_p_id . "' and  si_poster_id='" . $uid . "' ";
        $db->unbuffered_query($sql);

    }

    $sql = "Update cc" . $n . "_forum_topics set si_posts_count= si_posts_count -1  where  si_forum_id ='" . $f_id .
        "' AND  si_topic_id ='" . $ft_id . "'  ";
    $db->unbuffered_query($sql);
