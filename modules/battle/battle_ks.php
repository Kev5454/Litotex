...
function ks_battle($group_id, $land_id, $fromadmin)
{
    ...
    $sql = "SELECT cc" . $n . "_groups_inhalt . * , cc" . $n . "_soldiers . * FROM cc" . $n . "_groups_inhalt INNER JOIN cc" .
        $n . "_soldiers ON cc" . $n . "_groups_inhalt.type = cc" . $n . "_soldiers.tabless WHERE cc" . $n . "_soldiers.race ='" .
        $angreifer_race . "' AND cc" . $n . "_groups_inhalt.group_id ='" . $group_id . "'";
    ...
    while ($row_sol_vert = $db->fetch_array($result_sol_verteidiger))
    {
        ...
        if ($anzahl_einheiten > 0)
        {
            ...
            $kampfscript->setunits_verteidiger($id_vert, $AP * $anzahl_einheiten, $VP * $anzahl_einheiten, $AP, $VP, $anzahl_einheiten,
                $unit_type);
    ...
    foreach ($kampfscript->angreifer as $name => $value)
    {
        ...
        if ($fromadmin == 0)
        {
            $db->query("update cc" . $n . "_groups_inhalt set anzahl='" . $einheiten_new_count . "' where group_id='" . $group_id .
                "' and type='" . $einheiten_name_org . "'");
        }
    }
    ...
    if ($count_vert > 0)
    {
        foreach ($kampfscript->verteidiger as $name => $value)
        {
            ...
            if ($fromadmin == 0)
            {
                $db->query("update cc" . $n . "_countries set " . $einheiten_name_org . " ='" . $einheiten_new_count .
                    "' where islandid='" . $land_id . "'");
                $db->query("update cc" . $n . "_countries set " . $einheiten_name_org . " ='0' where islandid='" . $land_id . "' and " .
                    $einheiten_name_org . " <'0'");
            }
        }
    }
    ...
    $db->query("INSERT INTO cc" . $n .
        "_battle_archiv(ang_username,ang_land,vert_username,vert_land,battle_time,battle_url) VALUES ('$angreifer_name','$angreifer_land','$verteidiger_name','$verteidiger_land','" .
        time() . "','$battle_url')");
    ...
    $message_text = "Du wurdest angegriffen.<br>Den Kampfreport findest du hier:<br> [url=" . $Datei_url .
        "]Kampfreport Nr.:" . $kr_number . "[/url]";
    ...
    if ($all_einheiten_vert_count <= 0)
    {
        ...
        if ($max_steal > 0)
        {
            ...
            $result = $db->query("update cc" . $n . "_countries  set res1=res1-'" . $per_res1 . "',res2=res2-'" . $per_res2 .
                "',res3=res3-'" . $per_res3 . "',res4=res4-'" . $per_res4 . "' WHERE islandid='" . $land_id . "'");
            $result = $db->query("update cc" . $n . "_groups  set res1=res1+'" . $per_res1 . "',res2=res2+'" . $per_res2 .
                "',res3=res3+'" . $per_res3 . "',res4=res4+'" . $per_res4 . "' WHERE groupid='" . $group_id . "'");
