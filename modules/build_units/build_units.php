...
if ($action == "main")
{
    ...
    $result = $db->query("SELECT * FROM cc" . $n . "_soldiers where race = " . $userdata['rassenid'] .
        " AND (sol_type = 0 or sol_type = 2 or sol_type = 3) ORDER BY sid");
    while ($row = $db->fetch_array($result))
    {
        if ($userdata[$row['required']] >= $row['required_level'])
        {
            ...
            //    " AND (sol_type = 0 or sol_type = 2 or sol_type = 3) ORDER BY name");
    ...
    while ($row = $db->fetch_array($result))
    {
        ...
        if ($z == 0)
        {
            ...
            if ($anz_fertig != 0)
            {
                ...
                $db->query("UPDATE cc" . $n . "_create_sol SET anz= anz - '$anz_fertig', starttime='" . $starttime .
...
if ($action == "create")
{
    ...
    $result2 = $db->query("SELECT island_id, endtime, starttime, create_sol_id FROM cc" . $n .
        "_create_sol WHERE island_id = $userdata[activeid] AND sol_type = 0 ORDER BY create_sol_id DESC LIMIT 1");
...
if ($action == "del")
{
    ...
    else
    {
        ...
        $result = $db->query("SELECT * FROM cc" . $n . "_create_sol WHERE island_id = $userdata[activeid] AND (sol_type = 0 or sol_type = 2 or sol_type = 3) AND create_sol_id > '" .
