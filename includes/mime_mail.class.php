...
class MIME_Mail
{
    ...
    function sendMail()
    {
        ...
        if (empty($this->plainPart) && empty($this->htmlPart))
        {
            $err |= 4;
        }
        ...
        if ($this->checkSenderAddress)
        {
            ...
            if ($this->checkWWWAddress)
            {
                $okay = $this->isDauEmailOkay($this->internal_from) && $this->isDauEmailOkay($this->internal_replyto) && $this->
                    isDauEmailOkay($this->internal_envelop);
            }
            else
            {
                $okay = $this->isEmailOkay($this->internal_from) && $this->isEmailOkay($this->internal_replyto) && $this->isEmailOkay($this->
                    internal_envelop);
            }
...
" . $this->plainPart . "\n";
        ...
        $success = mail($this->to, $s, $body, $preheader . $this->mimeheader . $this->msprio_header . $this->prio_header . $this->
            xheader, "-f " . $f);
    ...
    function quotedPrintable($string)
    {
        ...
        for ($i = 0; $i < strlen($string); $i++)
        {
            ...
            if (($num > 126) || ($num == 61) || (($num < 33) && ($num != 13) && ($num != 10)) || ($num == 33) || ($num == 34) || ($num ==
                35) || ($num == 36) || ($num == 64) || ($num == 91) || ($num == 92) || ($num == 93) || ($num == 94) || ($num == 123) ||
                ($num == 124) || ($num == 125) || ($num == 126))
            {
                $hex = sprintf("%X", $num);
                if ($num < 17)
                {
