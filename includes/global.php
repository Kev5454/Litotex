...
else
{
    ...
    elseif ( is_file( '.' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'config.php' ) )
    {
        require ( '.' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'config.php' );
    }
    else
    {
...
if ( isset( $_SESSION['userid'] ) )
{
    ...
    while ( $row_land = $db->fetch_array( $result_land ) )
    {
        if ( ( ( int )$row_land['isbuilding'] ) == 1 && $row_land['endbuildtime'] <= time() )
        {
            ...
            {
                $sql .= "UPDATE cc" . $n . "_countries SET " . $row['tabless'] . "=" . $row['tabless'] . "+'1', bid='0', isbuilding='0', startbuildtime='0', endbuildtime='0' WHERE islandid='" . $row_land['islandid'] . "';";
            }
        }
        if ( ( ( int )$row_land['isexploring'] ) == 1 && $row_land['endexploretime'] <= time() )
        {
            ...
            if ( $db->num_rows( $result ) > 0 )
            {
                $sql .= "UPDATE cc" . $n . "_countries SET " . $row['tabless'] . "=" . $row['tabless'] . "+1, eid='0', startexploretime='0', endexploretime='0', isexploring='0' WHERE islandid='" . $row_land['islandid'] . "';";
            }
        }

        $result = $db->query( "SELECT * FROM cc" . $n . "_create_sol WHERE island_id = '" . $row_land['islandid'] . "' ORDER BY create_sol_id" );
        if ( $db->num_rows( $result ) > 0 )
        {
            while ( $row_create_sol = $db->fetch_array( $result ) )
            {
                ...
                {
                    $sql .= "UPDATE cc" . $n . "_countries SET " . $row_soldiers['tabless'] . "=" . $row_soldiers['tabless'] . "+'" . $row_create_sol['anz'] . "' WHERE islandid='" . $row_land['islandid'] . "';";
    ...
    $tpl->assign( 'IS_ADMIN',( $userdata['serveradmin'] == 1 ? true : false ) );
