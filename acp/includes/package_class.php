...
class package
{
     ...
     * @return bool
     */
    public function __construct($package_name, $ftp, $noaction = false)
    {
        $this->_resource = LITO_ROOT_PATH . $this->_resource;
        if ($ftp !== false)
        {
            $this->_ftp_connection = $ftp;
            if (!$this->_ftp_connection->lito_root) return false;
            else  $this->_ftp_connection_initialized = true;
        }
        if (!is_dir($this->_resource . '/' . $package_name)) return false;
    ...
    {
        if (!$this->_ftp_connection_initialized) return false;
        $dir = preg_replace('!^\.\/!', '', $dir);
        if ($this->_ftp_connection->exists($dir)) return true;
        if ($this->_ftp_connection->exists(dirname($dir)) || dirname($dir) == '.') return $this->_ftp_connection->mk_dir($dir);
        if ($this->_req_mkdir(dirname($dir))) return $this->_ftp_connection->mk_dir($dir);
    }
    /**
    ...
    {
        if (!$this->_ftp_connection->exists($this->_ftp_resource . '/' . $this->_pack_name . '/' . $file)) return false;
        $dest_dir = dirname($dest);
        if (!$this->_ftp_connection->exists($dest_dir)) $this->_req_mkdir($dest_dir);
        if (!$this->_ftp_connection->exists($dest_dir)) return false;
    ...
    {
        global $db, $n;
        if (!is_dir($this->_resource . '/' . $this->_pack_name)) return false;
        if (!file_exists($this->_resource . '/' . $this->_pack_name . '/setup.php')) return false;
        $error = 0;
        if ($this->_noaction == true) $this->_debug .=
                '<p>Die Installation wird nur <b>simuliert</b> dabei werden Fehler ausgegeben, Datenbank&auml;nderungen werden allerdings <b>nicht</b> getestet!</p>';
        ...
        if ($setup['modul_acp'] == 1) $prefx = 'acp/';
        else  $prefx = '';
        $backup = date('d.m.Y_H_i_s', time());
        $backup_done = false;
        $copyed = array();
        if (!$error && (isset($setup['move_core']) || $this->_ftp_connection->exists($prefx . 'modules/' . $setup['modul_name']) ||
            $this->_ftp_connection->exists($prefx . 'images/standard/' . $setup['modul_name']) || $this->_ftp_connection->exists($prefx .
            'themes/standard/' . $setup['modul_name']) || $this->_ftp_connection->exists($prefx . 'lang/' . $setup['modul_name'])))
        {
            $this->_debug .= '<p>Das Modul wurde installiert, es wird eine Sicherheitskopie unter /backup/' . $setup['modul_name'] .
                '-' . $backup . ' angelegt.</p>';
            if (!$this->_ftp_connection->exists('backup')) $this->_ftp_connection->mk_dir('backup');
            if (!$this->_ftp_connection->exists('backup')) return false;
            $this->_ftp_connection->mk_dir('backup/' . $setup['modul_name'] . '-' . $backup);
            if (!$this->_ftp_connection->exists('backup/' . $setup['modul_name'] . '-' . $backup)) return false;
            @$this->_ftp_connection->copy_req($prefx . 'modules/' . $setup['modul_name'], 'backup/' . $setup['modul_name'] . '-' . $backup .
                '/modules');
            @$this->_ftp_connection->copy_req($prefx . 'images/standard/' . $setup['modul_name'], 'backup/' . $setup['modul_name'] .
                '-' . $backup . '/images');
            @$this->_ftp_connection->copy_req($prefx . 'themes/standard/' . $setup['modul_name'], 'backup/' . $setup['modul_name'] .
                '-' . $backup . '/templates');
            @$this->_ftp_connection->copy_req($prefx . 'lang/' . $setup['modul_name'], 'backup/' . $setup['modul_name'] . '-' . $backup .
                '/languages');
            $backup_done = true;
        }
        if (!$error)
        {
            if (isset($setup['move_module_file']))
            {
                if (is_array($setup['move_module_file']))
                {
                    ...
                    {
                        if ($this->_move_file('modules/' . $setup['modul_name'] . '/' . $mod, $prefx . 'modules/' . $setup['modul_name'] . '/' .
                            $mod))
                        {
                        ...
                        {
                            $this->_debug .= "\n" . '<p>Folgende Moduldatei ' . $mod .
                                ' wurde wurde aufgrund eines Fehlers nicht in den Modulordner verschoben, &uuml;berpr&uuml;fen sie die existenz der Datei.</p>';
                            $error++;
                        }
                    }
                }
                ...
                {
                    if ($this->_move_file('modules/' . $setup['modul_name'] . '/' . $setup['move_module_file'], 'modules/' . $setup['modul_name'] .
                        '/' . $setup['move_module_file']))
                    {
                    ...
                    {
                        $this->_debug .= "\n" . '<p>Folgende Moduldatei ' . $setup['move_module_file'] .
                            ' wurde wurde aufgrund eines Fehlers nicht in den Modulordner verschoben, &uuml;berpr&uuml;fen sie die existenz der Datei.</p>';
                        $error++;
                    }
                }
            ...
            if (isset($setup['move_template_file']))
            {
                if (is_array($setup['move_template_file']))
                {
                    ...
                    {
                        if ($this->_move_file('themes/standard/' . $setup['modul_name'] . '/' . $mod, $prefx . 'themes/standard/' . $setup['modul_name'] .
                            '/' . $mod))
                        {
                ...
                {
                    if ($this->_move_file('themes/standard/' . $setup['modul_name'] . '/' . $setup['move_template_file'], 'themes/standard/' .
                        $setup['modul_name'] . '/' . $setup['move_module_file']))
                    {
                        $this->_debug .= "\n" . '<p>Folgende Templatedatei ' . $setup['move_template_file'] .
                            ' wurde in den Modulordner verschoben';
            ...
            if (isset($setup['move_lang_file']))
            {
                ...
                {
                    if ($this->_move_file('lang/' . $setup['modul_name'] . '/' . $setup['move_lang_file'], 'lang/' . $setup['modul_name'] .
                        '/' . $setup['move_module_file']))
                    {
                    ...
                    {
                        $this->_debug .= "\n" . '<p>Folgende Sprachdatei ' . $setup['move_lang_file'] .
                            ' wurde wurde aufgrund eines Fehlers nicht in den Modulordner verschoben, &uuml;berpr&uuml;fen sie die existenz der Datei.</p>';
                        $error++;
                    }
                }
            ...
            if (isset($setup['move_image_file']))
            {
                if (is_array($setup['move_image_file']))
                {
                    ...
                    {
                        if ($this->_move_file('images/standard/' . $setup['modul_name'] . '/' . $mod, $prefx . 'images/standard/' . $setup['modul_name'] .
                            '/' . $mod))
                        {
                ...
                {
                    if ($this->_move_file('images/standard/' . $setup['modul_name'] . '/' . $setup['move_image_file'], 'images/' . $setup['modul_name'] .
                        '/' . $setup['move_module_file']))
                    {
                    ...
                    {
                        $this->_debug .= "\n" . '<p>Folgende Bilddatei ' . $setup['move_image_file'] .
                            ' wurde wurde aufgrund eines Fehlers nicht in den Modulordner verschoben, &uuml;berpr&uuml;fen sie die existenz der Datei.</p>';
                        $error++;
                    }
                }
            ...
            if (isset($setup['db_update']) && !$error)
            {
                ...
                $ver_q = $db->query("SELECT `current_version` FROM `cc" . $n . "_modul_admin` WHERE `modul_name` = '" . $setup['modul_name'] .
                    "'");
                if ($db->num_rows($ver_q) == 0) $ver = '0.0.0';
                else
                {
                ...
                if (is_array($setup['db_update']))
                {
                    ...
                    {
                        if (compare_versions_sinus($ver, preg_replace("!^([0-9]\.[0-9]\.[0-9]);.*!", "$1", $mod)) != 1) continue;
                        $mod = preg_replace("!^([0-9]\.[0-9]\.[0-9]);!", "", $mod);
                        if ($this->_noaction) $this->_debug .= "\n" . '<p>' . $mod . '</p>';
                        else
                        {
                            if (mysql_query($mod)) $this->_debug .= "\n" . '<p>' . $mod . '</p>';
                            else
                            {
                                $this->_debug .= "\n" . '<p>Es ist ein <b>Fehler</b> bei folgendem Query "' . $mod . '" aufgetreten, Mysql meldet: "' .
                                    mysql_error() . '"</p>';
                                $error++;
                            }
                        }
                ...
                else
                {
                    ...
                    {
                        if ($db->query($setup['db_update'])) $this->_debug .= "\n" . '<p>' . $setup['db_update'] . '</p>';
                        else
                        {
                            $this->_debug .= "\n" . '<p>Es ist ein <b>Fehler</b> bei folgendem Query "' . $setup['db_update'] .
                                '" aufgetreten, Mysql meldet: "' . $db->error() . '"</p>';
                            $error++;
                        }
                    }
                }
                ...
                else
                {
                    ...
                    if ($db->num_rows($is_installed) == 0)
                    {
                        $db->query("INSERT INTO `cc" . $n .
                            "_modul_admin` (`modul_name`, `modul_description`, `disable_allowed`, `activated`, `startfile`, `current_version`, `acp_modul`, `show_error_msg`, `modul_type`, `new_upd_available`, `perm_lvl`) VALUES ('" .
                            $setup['modul_name'] . "', '" . $setup['modul_description'] . "', '" . $setup['modul_disable_allowed'] . "', '0', '" . $setup['modul_filename'] .
                            "', '" . $setup['modul_version'] . "', '" . $setup['modul_acp'] . "', '" . $setup['modul_show_error_msg'] . "', '" . $setup['modul_game_type'] .
                            "', '0', '" . $setup['modul_permission_level'] . "')");
                    }
                    else
                        if ($prefx == 'acp/')
                        {
                            $done = $db->query("UPDATE `cc" . $n . "_modul_admin` SET `modul_description` = '" . $setup['modul_description'] .
                                "', `current_version` = '" . $setup['modul_version'] . "' WHERE `modul_name` = '" . $setup['modul_name'] . "'");
                        }
                        else
                        {
                            $done = $db->query("UPDATE `cc" . $n . "_modul_admin` SET `modul_description` = '" . $setup['modul_description'] .
                                "', `current_version` = '" . $setup['modul_version'] . "' WHERE `modul_type` = '" . $setup['modul_game_type'] . "'");
                        }
                }
            }
        }
        ...
        {
            if ($error > 0) $this->_debug .= '<p><b>Es sind <u>' . $error .
                    '</u> Fehler aufgetreten! Bitte kontaktieren sie den Entwickler des Moduls "' . $setup['modul_autor'] .
                    '" per E-Mail an "<a href="mailto:' . $setup['modul_autor_mail'] . '">' . $setup['modul_autor_mail'] .
                    '</a>" falls sie die Dateien nicht manipuliert haben.</b></p>';
            if ($backup_done)
            {
                ...
                foreach ($copyed as $file => $type)
                {
                    ...
                    {
                        case 'module':
                            $this->_ftp_connection->mv($prefx . 'modules/' . $setup['modul_name'] . '/' . $file, 'acp/tmp/' . $this->_pack_name .
                                '/modules/' . $setup['modul_name'] . '/' . $file);
                            break;
                        case 'image':
                            $this->_ftp_connection->mv($prefx . 'images/standard/' . $setup['modul_name'] . '/' . $file, 'acp/tmp/' . $this->
                                _pack_name . '/images/standard/' . $setup['modul_name'] . '/' . $file);
                            break;
                        case 'lang':
                            $this->_ftp_connection->mv($prefx . 'lang/' . $setup['modul_name'] . '/' . $file, 'acp/tmp/' . $this->_pack_name .
                                '/lang/' . $setup['modul_name'] . '/' . $file);
                            break;
                        case 'template':
                            $this->_ftp_connection->mv($prefx . 'themes/standard/' . $setup['modul_name'] . '/' . $file, 'acp/tmp/' . $this->
                                _pack_name . '/themes/standard/' . $setup['modul_name'] . '/' . $file);
                            break;
                        case 'core':
                ...
                $this->_ftp_connection->mv('backup/' . $setup['modul_name'] . '-' . $backup . '/templates', $prefx . 'themes/standard/' .
                    $setup['modul_name']);
    ...
    public function deinstall()
    {
        ...
        if ($setup['modul_type'] == 'acp') $prefx = 'acp/';
        else  $prefx = '';
        $backup = date('d.m.Y_H_i_s', time());
        if (!$this->_ftp_connection->exists('backup')) $this->_ftp_connection->mk_dir('backup');
        if (!$this->_ftp_connection->exists('backup')) return false;
        $this->_ftp_connection->mk_dir('backup/' . $setup['modul_name'] . '-' . $backup);
        if (!$this->_ftp_connection->exists('backup/' . $setup['modul_name'] . '-' . $backup)) return false;
        @$this->_ftp_connection->mv($prefx . 'modules/' . $setup['modul_name'], 'backup/' . $setup['modul_name'] . '-' . $backup .
            '/modules');
        @$this->_ftp_connection->mv($prefx . 'images/standard/' . $setup['modul_name'], 'backup/' . $setup['modul_name'] . '-' .
            $backup . '/images');
        @$this->_ftp_connection->mv($prefx . 'themes/standard/' . $setup['modul_name'], 'backup/' . $setup['modul_name'] . '-' .
            $backup . '/templates');
        @$this->_ftp_connection->mv($prefx . 'lang/' . $setup['modul_name'], 'backup/' . $setup['modul_name'] . '-' . $backup .
            '/languages');
        if (isset($setup['de_db_update']) && !$error)
        {
            ...
            else
            {
                ...
                {
                    if (mysql_query($setup['de_db_update'])) $this->_debug .= "\n" . '<p>' . $setup['de_db_update'] . '</p>';
                    else
                    {
                        $this->_debug .= "\n" . '<p>Es ist ein <b>Fehler</b> bei folgendem Query "' . $setup['de_db_update'] .
                            '" aufgetreten, Mysql meldet: "' . mysql_error() . '"</p>';
                        $error++;
                    }
                }
            }
