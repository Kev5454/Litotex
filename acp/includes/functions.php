...
function template_out($template_name, $from_modulname)
{
    ...
    $tpl->config_load(LITO_LANG_PATH . $from_modulname . '/lang_' . $lang_suffix . '.php');
...
{
    global $db, $n, $op_set_n_res1, $op_set_n_res2, $op_set_n_res3, $op_set_n_res4;

    $sel_name = trim($sel_name);
    ...
    while ($row = $db->fetch_array($result))
    {
        ...
        $select = ($sel_name == trim($row['tabless']) ? 'selected' : '');
        $out .= "<option value=\"" . $row['tabless'] . "\" " . $select . ">" . $name_description . "</option>";
    }
    $out .= "</select>";
    return $out;
}
...
{
    $out = "<select name=\"$name\" class=\"combo\">";
    $out .= " <option value=\"0\" " . ($sel_type == 0 ? 'selected' : '') . ">Angriff</option>";
    $out .= " <option value=\"1\" " . ($sel_type == 1 ? 'selected' : '') . ">Verteidigung</option>";
    $out .= " <option value=\"2\" " . ($sel_type == 2 ? 'selected' : '') . ">Landerweierung</option>";
    $out .= " <option value=\"3\" " . ($sel_type == 3 ? 'selected' : '') . ">Spion</option>";
    $out .= "</select>";
    return $out;
}
...
function make_race_choice($name, $sel_id)
{
    ...
    while ($row = $db->fetch_array($result))
    {
        $select = ($sel_id == (int)$row['rassenid'] ? 'selected' : '');
        $out .= "<option value=\"" . $row['rassenid'] . "\" " . $select . ">" . $row['rassenname'] . "</option>";
    }
    $out .= "</select>";
    return $out;
}
