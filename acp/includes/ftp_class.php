...
class ftp
{
     ...
     */
    private $rootdir = './';
    public function __construct($host = '', $user = '', $password = '', $rootdir = './', $port = 21)
    {
        ...
        else
        {
            ...
            if (!@$this->_connection = ftp_connect($this->_host, $this->_port)) return false;
            if (!@ftp_login($this->_connection, $this->_user, $this->_password)) return false;
            $this->connected = true;
    ...
    public function mk_dir($dir)
    {
        ...
        if ($this->_method == 'PHP') return @mkdir($this->rootdir . '/' . $dir);
        else  return @ftp_mkdir($this->_connection, $dir);
    }
    /**
    ...
    public function rm_dir($dir)
    {
        ...
        if (!$this->connected) return false;
        if (!$this->exists($dir)) return false;
        if ($this->_method == 'PHP')
        {
    ...
    public function mv($file, $dest)
    {
        ...
        else  return @ftp_rename($this->_connection, $file, $dest);
    }
    /**
    ...
    public function list_files($dir = './')
    {
        ...
        if ($this->_method == 'PHP')
        {
            if (!is_dir($this->rootdir . '/' . $dir)) return false;
    ...
    public function rm_file($file)
    {
        ...
        if ($this->_method == 'PHP')
        {
            if (!is_file($this->rootdir . '/' . $file)) return false;
            return @unlink($this->rootdir . '/' . $file);
        }
        else  return @ftp_delete($this->_connection, $file);
    }
    /**
    ...
    public function chown_perm($ch, $file)
    {
        ...
        else  return @ftp_chmod($this->_connection, $ch, $file);
    }
    /**
    ...
    {
        if (!$this->connected) return false;
        if ($overwrite && $this->exists($file)) return false;
        if ($this->_method == 'PHP')
        {
    ...
    public function copy_req($source, $dest)
    {
        ...
        if (!$this->list_files($source)) return false;
        if (!$this->exists($dest))
            if (!$this->mk_dir($dest)) return false;
        $source_files = $this->list_files($source);
        foreach ($source_files as $file)
        {
            if ($file == '.' || $file == '..' || $file == $source . '/.' || $file == $source . '/..') continue;
            if (!preg_match('!' . preg_replace('!^\/!', '', $source) . '!', $file)) $file = $source . '/' . $file;
            if ($this->isdir($file)) $this->copy_req($file, str_replace($source, $dest, $file));
            else
            {
    ...
    public function isdir($file)
    {
        ...
        {
            if (ftp_size($this->_connection, $file) == -1) return true;
            return false;
        }
    }
    /**
    ...
    {
        if ($this->_method == 'PHP') $this->connected = false;
        else
        {
            if (!$this->connected) return false;
    ...
    public function exists($file)
    {
        ...
        else
        {
            ...
            $file = basename($file);
            if ($dir == '.') $dir = '';
            if ($dir == '' && count($this->list_files($dir)) == 0) $dir = '.';
            if (@in_array($file, @$this->list_files($dir), '/')) return true;
            else  return false;
        }
    }
    /**
    ...
    private function _php_dir_delete($verz, $folder = array())
    {
        ...
        while ($dir_file = readdir($fp))
        {
            ...
            if (is_dir($neu_file)) $folder = $this->_php_dir_delete($neu_file, $folder);
    ...
    private function _php_all_delete($dir_file)
    {
        if (is_dir($dir_file))
        {
            ...
            foreach ($array as $elem) rmdir($elem);
        }
