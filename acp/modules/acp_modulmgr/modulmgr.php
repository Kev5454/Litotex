...
elseif ($action == "get_info")
{
    ...
    if (is_file($setup_filename))
    {
        ...
        {
            $more_info .= "<br><a href=\"" . $ini_array['info']['modul_autor_website'] . "\" target=\"_blank\">" . $ini_array['info']['modul_autor_website'] .
                "</a>";
        }
        else
        {
    ...
    {
        $more_info = "Nicht Verfügbar";
    }

    $upd_info = (intval($row['new_upd_available']) == 0 ? "Kein Update vorhanden." : "Update verfügbar!");
...
elseif ($action == 'upload')
{
    ...
    if ($_FILES['upload_mod']['type'] != 'application/zip' && $_FILES['upload_mod']['type'] !=
        'application/x-zip-compressed' && $_FILES['upload_mod']['type'] != 'application/x-compressed' && $_FILES['upload_mod']['type'] !=
        'multipart/x-zip' && $_FILES['upload_mod']['type'] != 'application/x-zip')
    {
    ...
    {
        if (!in_array($node, $old_tree)) $new = $node;
    }
    if (!isset($new))
    {
...
elseif ($action == 'remote_update')
{
    ...
    if (!$pm->initialized) die('Schwerer Fehler!');
    $pm->install();
    ...
    while ($mod = readdir($scandir))
    {
        ...
        if (is_file($setup_filename))
        {
            ...
            if (is_modul_installed($new_modul_name, $new_modul_version) == 0)
            {
                $new_found[] = array(
                    $new_modul_name,
                    $ini_array['info']['modul_description'],
                    $new_modul_version,
                    $mod,
                    );
                if ($new_type == 1)
...
{
    $m_name = filter_var($_GET['id'], FILTER_SANITIZE_STRING);
    ...
    if (file_exists($setup_filename))
    {
        ...
        if (!isset($ftp) || !is_a($ftp, 'ftp')) $ftp = new ftp($ftphost, $ftpuser, $ftppassword, $ftproot, $ftpport);
